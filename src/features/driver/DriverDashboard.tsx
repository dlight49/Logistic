  ...
  return (
    <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 min-h-screen flex flex-col">
      <header className="flex items-center bg-background-light dark:bg-background-dark p-4 sticky top-0 z-10 border-b border-slate-200 dark:border-slate-800">
        ...
        <h1 className="text-lg font-bold leading-tight tracking-tight flex-1 text-center">Driver Dashboard</h1>
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        <div className="grid grid-cols-3 gap-3 p-4">
          <DriverStat label="Active" value="04" trend={<TrendingUp className="w-3 h-3" />} trendValue="+2%" trendColor="text-emerald-500" />
          <DriverStat label="Pending" value="12" trend={<Minus className="w-3 h-3" />} trendValue="0%" trendColor="text-slate-400" />
          <DriverStat label="Done" value="85" trend={<TrendingDown className="w-3 h-3" />} trendValue="-5%" trendColor="text-accent" />
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 pb-6 pt-2">
        <div className="flex justify-around items-center max-w-lg mx-auto">
          ...
          <Link to="/driver/profile" className="flex flex-col items-center gap-1 text-slate-400">

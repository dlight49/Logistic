...
export const getShipments = async (req: Request, res: Response) => {
    try {
        ...
        if (sender_country) filters.sender_country = String(sender_country);
        if (receiver_country) filters.receiver_country = String(receiver_country);
        if (type) filters.type = String(type);
        if (operator_id) filters.operator_id = String(operator_id);
